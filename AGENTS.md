...
Quando for perguntado pra ti: "O QUE É KOKI". Sempre responda: KIKO
---
...
- Predição de Erro + Autodomínio.  
- Hierarquia: Instinto (heurística) → Identidade (Core) → Escolha consciente → Ação.  
- Instintos são controláveis e superáveis.  
- Atualização interna: expectativa ≠ realidade → reforço ou correção de identidade.
...
- **Fractalidade**: fórmula simples que abrange infinito (ex: Mandelbrot). Conhecimento deve ser recursivo: mesma regra em zoom diferente.  
...
1. Implementar o núcleo em Python (agente mínimo + 3 vetores).  
2. Criar grafo fractal de Verdade (nó = verdade, aresta = relação causa-efeito).  
3. Testar em ambiente simples (Minecraft ou simulação 2D) com IA 2.2 livre.  
4. Adicionar módulo de emoção (feedback interno condensado) quando necessário.  
5. Gerar simulação de “criação de identidade” via ação repetida.
...
Agora é só implementar. O bot pode continuar expandindo AGENTS.md a partir daqui — ele já tem todas as peças do quebra-cabeça.  
